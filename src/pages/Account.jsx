      ...
      <div className='container'>
        <div className='account-container'>
          ...
          <div className='account-info'>
            ...
            <p> Pronouns: {userData.pronouns}
              {!showPronounForm && <button onClick={() => {
                togglePronounForm(!showPronounForm)
              }}>Change Pronouns</button>}
              {showPronounForm && <input placeholder="Pronouns" onChange={(event) => {
                setPronoun(event.target.value);
              }} />}
              {showPronounForm &&
                <button onClick={updatePronouns}>Submit</button>}
            </p>
            <p> Number: {userData.phoneNumber}
              {!showNumberForm && <button onClick={() => {
                toggleNumberForm(!showNumberForm)
              }}>Change Number</button>}
              {showNumberForm &&
                <input placeholder="Number" type="tel" onChange={(event) => {
